...
async function handleSubmit() {
  ...
  try {
    const response = await axios.post(
      'https://d9tej631xf.execute-api.ap-southeast-7.amazonaws.com/prod/notes/edit',
      {
        name: name,
        noteId: noteID.value,
    ...
    if (response.status === 200) {
      dialogMessage.value = 'Note updated successfully!'
...
<template>
  <div class="page">
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div class="form-group">
        <label>Note ID</label>
        <input v-model="noteID" type="text" placeholder="Enter note ID to edit" required />
      </div>
      <div class="form-group">
        <label>Note Content</label>
        <textarea v-model="content" placeholder="Enter new note content" rows="6" required></textarea>
      </div>
