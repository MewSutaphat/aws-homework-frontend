...
async function handleSubmit() {
  ...
  try {
    const response = await axios.post(
      'https://d9tej631xf.execute-api.ap-southeast-7.amazonaws.com/prod/notes/delete',
      {
        name: name,
        noteId: noteID.value,
      },
    ...
    if (response.status === 200) {
      dialogMessage.value = 'Note deleted successfully!'
...
<template>
  <div class="page">
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div class="form-group">
        <label>Note ID</label>
        <input v-model="noteID" type="text" placeholder="Enter note ID to delete" required />
      </div>
      <button type="submit" class="submit-btn submit-btn--danger">Submit</button>
    </form>
  </div>
